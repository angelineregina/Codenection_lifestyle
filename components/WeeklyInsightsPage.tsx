...
import { weeklyInsights } from '@/data/demoData';
...
function MoodStressChart() {
  const moodPoints = weeklyInsights.moodTrend.map((point) => ({ label: point.day, value: point.value }));
  const stressPoints = weeklyInsights.stressTrend.map((point) => ({ label: point.day, value: point.value }));
  ...
  return (
    <main className="app-main insights-page">
      <header className="insights-header">
        ...
        <span className="insights-period-label">{weeklyInsights.period}</span>
      </header>
      ...
      <section className="insights-content">
        <section className="insights-card" aria-labelledby="weekly-workload-title">
          ...
          <TrendChart points={weeklyInsights.workloadTrend} minValue={60} maxValue={115} ariaLabel="Weekly workload percentage trend" highlightWindow />
          ...
          <p className="insights-takeaway"><Lightbulb size={14} />{weeklyInsights.workloadTakeaway}</p>
        </section>
        ...
        <section className="insights-card" aria-labelledby="recovery-title">
          ...
          <div className="recovery-chart" aria-label="Daily recovery minutes across the week">
            {weeklyInsights.recoveryTrend.map((point) => <div className="recovery-bar-wrap" key={point.label}><strong>{point.minutes}</strong><span className="recovery-bar" style={{ height: `${Math.max(12, (point.minutes / 90) * 68)}px` }} /><small>{point.label}</small></div>)}
          </div>
          <p className="insights-takeaway"><Lightbulb size={14} />{weeklyInsights.recoveryTakeaway}</p>
        </section>
        ...
        <section className="insights-card" aria-labelledby="dimensions-title">
          ...
          <div className="weekly-dimension-list">
            {weeklyInsights.dimensions.map((dimension) => <div className="weekly-dimension-row" key={dimension.id}><div><span>{dimension.label}</span><strong>{dimension.value}%</strong></div><span className="weekly-dimension-track"><i style={{ width: `${dimension.value}%` }} /></span></div>)}
          </div>
        </section>

        <section className="weekly-takeaway-card" aria-labelledby="weekly-takeaway-title">
          <p className="section-kicker">Section E</p><h2 id="weekly-takeaway-title">Weekly Takeaway</h2><p>{weeklyInsights.weeklyTakeaway}</p>
