...
export function CapacityCard({ capacity, onClick }: { capacity: CapacityData; onClick: () => void }) {
  return (
    <button className="summary-card capacity-card" type="button" onClick={onClick}>
      <span className="card-heading"><span>Weekly Load</span><ChevronRight size={16} aria-hidden="true" /></span>
      <span className="capacity-number">{capacity.score}%</span>
      <span className="capacity-caption">average this week</span>
      <span className="capacity-chart" aria-label="Weekly load history bar chart">
